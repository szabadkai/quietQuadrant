...
import { BaseGameSystem, EventBus, ServiceContainer, SystemRegistry } from '../game/systems';
    ...
    fc.assert(
      fc.property(
        ...
        (systemId, dependencies, _priority) => {
          ...
          class MockSystem extends BaseGameSystem {
          ...
          const hasCorrectProperties =
            ...
            system.dependencies.every((dep) => typeof dep === 'string') &&
        ...
        (systemConfigs) => {
          ...
          try {
            ...
            for (const config of systemConfigs) {
              ...
              const availableSystemIds = systemConfigs.map((c) => c.systemId);
              const hasAllDependencies = config.dependencies.every((dep) =>
            ...
            for (const [systemId] of allSystems) {
              const retrievedSystem = registry.getSystem(systemId);
              if (!retrievedSystem || !validateSystemInterface(retrievedSystem)) {
                return false;
              }
            }
        ...
        (serviceConfigs) => {
          ...
          try {
            ...
            for (const config of serviceConfigs) {
              container.registerInstance(config.serviceId, config.value);
            }
            ...
            const expectedIds = serviceConfigs.map((c) => c.serviceId);
  ...
  test('Property 4c: EventBus interface consistency', () => {
    fc.assert(
      fc.property(
        fc.array(
          fc.record({
            event: fc.constantFrom('test:event1', 'test:event2', 'test:event3'),
            data: fc.oneof(fc.string(), fc.integer(), fc.record({ value: fc.integer() })),
          }),
        ...
        (eventConfigs) => {
          ...
          try {
            ...
            for (const config of eventConfigs) {
              const unsubscribe = eventBus.on(config.event as keyof EventBusEvents, (data) => {
                receivedEvents.push({
                  event: config.event,
                  data,
                });
            ...
            // Emit events
            for (const config of eventConfigs) {
              eventBus.emit(config.event as keyof EventBusEvents, config.data);
            }
            ...
            for (let i = 0; i < receivedEvents.length; i++) {
              ...
              if (received.event !== expected.event || received.data !== expected.data) {
                return false;
              }
            }

            // Listener count should be accurate
            const uniqueEvents = [...new Set(eventConfigs.map((c) => c.event))];
            for (const event of uniqueEvents) {
              const count = eventBus.listenerCount(event as keyof EventBusEvents);
              const expectedCount = eventConfigs.filter((c) => c.event === event).length;
