...
class SoundManager {
    ...
    private volumes: Pick<
        Settings,
        "masterVolume" | "musicVolume" | "sfxVolume" | "muteAll" | "muteMusic"
    > = {
        masterVolume: 0.6,
        musicVolume: 0.5,
        sfxVolume: 0.7,
    ...
    playSfx(key: SfxKey) {
        ...
        switch (key) {
            ...
            case "xpPickup":
                ...
                break;
            case "playerHit":
                // Shield bump sound - metallic clang with low thud
                this.playTone(ctx, {
                ...
                this.playNoise(ctx, 0.12, 0.2, 800);
                break;
            case "playerDeath":
                // Explosion with descending tone
    ...
    private applyVolumes() {
        ...
        const master = this.volumes.muteAll
            ? 0
            : clampVolume(this.volumes.masterVolume);
        const music =
            this.volumes.muteAll || this.volumes.muteMusic
                ? 0
                : clampVolume(this.volumes.musicVolume);
        const sfx = this.volumes.muteAll
            ? 0
            : clampVolume(this.volumes.sfxVolume);
