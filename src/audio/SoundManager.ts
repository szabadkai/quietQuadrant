...
class SoundManager {
  ...
  private playExclusiveTrack(url?: string) {
    const resolvedUrl =
      url ?? FIRST_LEVEL_TRACK_URL ?? OTHER_LEVEL_TRACKS[0] ?? TITLE_TRACK_URL ?? ALL_TRACK_URLS[0];
  ...
  setSettings(
    settings: Pick<Settings, 'masterVolume' | 'musicVolume' | 'sfxVolume' | 'muteAll' | 'muteMusic'>
  ) {
    this.volumes = {
  ...
  private applyVolumes() {
    ...
    const music =
      this.volumes.muteAll || this.volumes.muteMusic ? 0 : clampVolume(this.volumes.musicVolume);
