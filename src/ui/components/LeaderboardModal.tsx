...
export const LeaderboardModal = () => {
	...
	const weeklyBest = seasonInfo?.seedId
		...
		return (
			<div className="run-details">
				<div className="pill-row">
					...
					{synergies.length > 0 && (
						<span className="pill achievement">
				...
				<div className="swatch-grid">
					{run.upgrades.map((u) => {
						...
						return (
							<div
								key={u.id}
								className={`upgrade-swatch ${def?.rarity ?? "common"}`}
							>
								...
								<div className="swatch-desc">
									{def?.description ?? "Unknown upgrade"}
								</div>
							</div>
						);
					})}
	...
	const rows =
		tab === "weekly"
			? top10.filter(
					(r) =>
						r.seedId === seasonInfo?.seedId || r.runId.startsWith("empty-"),
				)
			: top10;
					...
					<>
						...
						<div className="leaderboard-card top-list">
							...
							<div className="leaderboard-list">
								{rows.map((run, idx) => {
									const isEmpty =
										run.wavesCleared === 0 && run.runId.startsWith("empty-");
									const expanded = expandedRunId === run.runId;
									return (
										<div
											key={`${tab}-${run.runId}`}
												...
												{isEmpty ? (
													...
													<>
														<span className="metric">
															Wave {run.wavesCleared}
														</span>
														<span className="tiny">
															{formattedTime(run.durationSeconds)} ·{" "}
															{run.enemiesDestroyed} enemies
														</span>
							...
							{tab === "weekly" && !weeklyBest && (
								<div className="tiny">
									No runs on this seed yet. Finish a run to claim it.
								</div>
							)}
							{tab === "all" && !bestRun && (
								<div className="tiny">No runs recorded yet.</div>
							)}
						</div>
					</>
				)}
