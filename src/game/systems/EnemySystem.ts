...
export class EnemySystem extends BaseGameSystem implements IEnemySystem {
    ...
    startWave(waveIndex: number): void {
        ...
        this.currentWave = {
            index: waveIndex,
            active: true,
            remainingEnemies: spawns.reduce(
                (sum, spawn) => sum + spawn.count,
                0
            ),
    ...
    getEnemiesGroup(): Phaser.Physics.Arcade.Group | undefined {
        return this.enemies;
    }

    getEnemyBulletsGroup(): Phaser.Physics.Arcade.Group | undefined {
        ...
        this.currentWave.spawnQueue.forEach((spawn) => {
            if (spawn.spawnTime <= now && spawn.count > 0) {
                for (let i = 0; i < spawn.count; i++) {
                    this.spawnEnemy(spawn.type as EnemyKind);
                }
    ...
    private checkWaveCompletion(): void {
        ...
        eventBus.emit("wave:enemy-count-changed", {
            ...
            total: this.currentWave.spawnQueue.reduce(
                (sum, spawn) => sum + spawn.count,
                0
            ),
        });
    ...
    private getNearestPlayerPosition(
        _x: number,
        _y: number
    ): Phaser.Math.Vector2 | null {
        // Get player position from the scene's player system
        // The player position is stored in the scene's children
        const scene = this.getScene();

        // Find player sprite by texture key
        const playerSprite = scene.children.list.find((child) => {
            const img = child as Phaser.Physics.Arcade.Image;
            return img.texture?.key === "player" && img.active;
        }) as Phaser.Physics.Arcade.Image | undefined;

        if (playerSprite) {
            return new Phaser.Math.Vector2(playerSprite.x, playerSprite.y);
        }
