...
export class MainScene extends Phaser.Scene {
	...
	private readControlsForPilot(
		...
		if (binding.type === "gamepad") {
			...
			return (
				adapter?.update(binding.id, binding.index) ?? this.emptyControlState()
			);
		}
	...
	private spawnBullet(config: {
		...
		bullet.setTint(
			isRailgun ? COLOR_OVERLOAD : config.charged ? COLOR_CHARGE : COLOR_ACCENT,
		);
		return bullet;
	}
	...
	private spawnDashSparkExplosion(
		...
		if (this.dashSparkConfig.shards <= 0 || this.dashSparkConfig.damage <= 0)
			return;
		const baseDir =
			dashDir.lengthSq() > 0
	...
	private handleWaveIntermission(dt: number) {
		...
		if (secondsLeft !== this.lastCountdownBroadcast) {
			...
			useRunStore
				.getState()
				.actions.setWaveCountdown(secondsLeft, this.pendingWaveIndex + 1);
		}
	...
	private computeWaveScaling(index: number) {
		...
		const countScale =
			(overflow > 0 ? 1 + overflow * 0.35 : 1) * this.modeEnemyCountMultiplier;
	...
	private spawnBoss() {
		...
		const health =
			base.health * (tuning.healthMultiplier ?? 1) * this.enemyHealthScale;
	...
	private handlePlayerDamage(
		...
		let remaining =
			(isContact ? amount + 0.5 : amount) * contactMultiplier * armorMultiplier;
