...
import { getUpgradeDefinition, UPGRADE_CATALOG, UPGRADE_RARITY_ODDS } from '../../config/upgrades';
...
export class MainScene extends Phaser.Scene {
  ...
  startNewRun(
    ...
    if (!this.physics || !this.physics.world) {
      this.events.once(Phaser.Scenes.Events.CREATE, () =>
        this.startNewRun(seedId, seedValue, affix, bossOverride, options)
      );
      return;
    }
    this.seedId = seedId;
  ...
  update(_: number, delta: number) {
    ...
    const controlSnapshots = activePilots.map((pilot) => {
      ...
      } else if (binding.type === 'keyboardMouse' && this.isPointerDown()) {
    ...
    if (
      ...
      const hasNextWave = this.infiniteMode || this.waveIndex < WAVES.length - 1;
      if (hasNextWave) {
  ...
  private checkSynergies() {
    SYNERGY_DEFINITIONS.forEach((syn) => {
      ...
      const ready = syn.requires.every((req) => (this.upgradeStacks[req] ?? 0) > 0);
      if (ready) {
  ...
  private enableSynergy(id: string) {
    ...
    const anchor = this.playerState?.sprite ?? this.playerTwoState?.sprite ?? null;
    if (anchor && !this.lowGraphics) {
      this.spawnBurstVisual(anchor.x, anchor.y, 38 * OBJECT_SCALE, COLOR_OVERLOAD, 0.85);
    }
  }
  ...
  private createStarfieldTextures() {
    ...
    layers.forEach((layer) => {
      ...
      for (let i = 0; i < layer.count; i++) {
        ...
        const radius = Phaser.Math.FloatBetween(layer.radius[0], layer.radius[1]);
  ...
  private createStarfieldLayers() {
    ...
    defs.forEach((def) => {
      const sprite = this.add
        .tileSprite(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH, GAME_HEIGHT, def.key)
        .setDepth(def.depth)
        .setAlpha(def.alpha);
  ...
  private createPlayfieldBackdrop() {
    ...
    this.playfieldBackdrop = this.add
      .rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH - 16, GAME_HEIGHT - 16, 0x0b0f13, 0.94)
  ...
  private playBossIntroPulse() {
    if (this.bossIntroOverlay) {
      ...
      this.bossIntroOverlay.setFillStyle(this.bossIntroColor, this.bossIntroOverlay.fillAlpha);
      this.tweens.add({
  ...
  private getPilotBySprite(sprite: Phaser.Physics.Arcade.Image): PilotRuntime | undefined {
  ...
  private isPilotActive(pilot?: PilotRuntime) {
    ...
    return pilot.sprite.visible && pilot.sprite.active && !!body?.enable && pilot.sprite.alpha > 0;
  }
  ...
  private getActivePilots(): PilotRuntime[] {
    ...
    if (this.runMode === 'twin' && this.isPilotActive(this.playerTwoState) && this.playerTwoState) {
  ...
  private readControlsForPilot(pilot: PilotRuntime, binding: ControlBinding): GamepadControlState {
    if (this.runMode !== 'twin') {
      return this.gamepadAdapter?.update() ?? pilot.gamepadState ?? this.emptyControlState();
    }
    if (binding.type === 'gamepad') {
      const adapter = pilot.id === 'p1' ? this.gamepadAdapter : this.gamepadAdapterP2;
      return adapter?.update(binding.id, binding.index) ?? this.emptyControlState();
    }
  ...
  private setupCollisions() {
    this.physics.add.collider(
      ...
      this.enemies,
      (a, b) =>
        this.handleEnemyCollide(a as Phaser.Physics.Arcade.Image, b as Phaser.Physics.Arcade.Image),
      undefined,
      this
    );
    ...
    this.physics.add.overlap(
      ...
      this.enemies,
      (playerObj, enemy) => {
        const pilot = this.getPilotBySprite(playerObj as Phaser.Physics.Arcade.Image);
        if (!pilot) return;
        this.handlePlayerDamage(pilot, enemy as Phaser.Physics.Arcade.Image, 1, true);
      },
      undefined,
      this
    );
    this.physics.add.overlap(
      ...
      (playerObj, bullet) => {
        ...
        if (!pilot) return;
        bullet.destroy();
        this.handlePlayerDamage(pilot, bullet as Phaser.Physics.Arcade.Image, 1, false);
      },
      undefined,
      this
    );
  ...
  private resetState() {
    ...
    if (!this.player || !this.player2 || !this.playerState || !this.playerTwoState) {
      this.spawnPlayers();
    }
    ...
    this.baseDifficulty =
      typeof settings.difficultyMultiplier === 'number' ? settings.difficultyMultiplier : 1;
    ...
    const resetPilot = (
      ...
      const body = pilot.sprite.body as Phaser.Physics.Arcade.Body | undefined;
      if (body) {
        body.enable = active;
    ...
    resetPilot(this.playerTwoState, { x: center.x + offset, y: center.y }, twinActive);
  ...
  private handlePlayerMovement(
    ...
    } else if (binding.type === 'keyboardMouse' || this.runMode !== 'twin') {
    ...
    const dashPressed =
      ...
      (binding.type === 'keyboardMouse' && Phaser.Input.Keyboard.JustDown(this.wasd.SHIFT));
    if (dashPressed) {
  ...
  private isPointerDown() {
    return this.input.activePointer?.isDown === true || this.input.mousePointer?.isDown === true;
  }
  ...
  private updateChargeState(pilot: PilotRuntime, dt: number, fireHeld: boolean) {
    if (this.capacitorConfig.stacks === 0) return;
    if (fireHeld) {
      pilot.charge.holdMs = Math.min(this.capacitorConfig.idleMs, pilot.charge.holdMs + dt * 1000);
  ...
  private updateMomentum(
    ...
    if (moving) {
      ...
      pilot.momentum.timerMs = Math.max(0, pilot.momentum.timerMs - dt * 800);
    }
    const ratio = Phaser.Math.Clamp(pilot.momentum.timerMs / this.momentumConfig.timeToMaxMs, 0, 1);
  ...
  private tryDash(pilot: PilotRuntime, dir: Phaser.Math.Vector2) {
    ...
    pilot.ability.activeUntil = this.time.now + pilot.ability.dashDurationMs;
    pilot.invulnUntil = pilot.ability.activeUntil;
    pilot.ability.nextDashAt = this.time.now + pilot.ability.dashCooldownMs;
    this.spawnDashTrail(new Phaser.Math.Vector2(pilot.sprite.x, pilot.sprite.y), dashDir);
    this.spawnAfterimageShots(dashDir, pilot);
    this.spawnDashSparkExplosion(new Phaser.Math.Vector2(pilot.sprite.x, pilot.sprite.y), dashDir);
  }
  ...
  private handleShooting(
    ...
    const adjustedFireRate =
      this.playerStats.fireRate * fireRateBonus * (1 + this.getBerserkBonus());
    ...
    for (let i = 0; i < spreadCount; i++) {
      const offset =
        spreadCount <= 1 ? 0 : (i - (spreadCount - 1) / 2) * Phaser.Math.DegToRad(spreadStepDeg);
  ...
  private spawnBullet(config: {
    ...
    const bullet = this.bullets.get(config.x, config.y, 'bullet') as Phaser.Physics.Arcade.Image;
    if (!bullet) return null;
    const sizeScale = (config.sizeMultiplier ?? 1) * this.projectileScale * OBJECT_SCALE;
    const isHeavy = this.neutronCoreConfig.active;
    const isRailgun = config.charged === true && this.activeSynergies.has('railgun');
    bullet.setActive(true);
    bullet.setVisible(true);
    bullet.setScale(sizeScale);
    const body = bullet.body as Phaser.Physics.Arcade.Body;
    const projectileSpeed =
      this.playerStats.projectileSpeed * (isHeavy ? this.neutronCoreConfig.speedMultiplier : 1);
    if (isHeavy) {
    ...
    body.setVelocity(config.dir.x * projectileSpeed, config.dir.y * projectileSpeed);
    bullet.setRotation(config.dir.angle());
    bullet.setData('pierce', this.getPierceWithSynergy(config.pierce, config.charged === true));
    ...
    bullet.setData(
      'expireAt',
      this.quantumConfig.active ? this.time.now + this.quantumConfig.projectileLifetimeMs : null
    );
    bullet.setTint(isRailgun ? COLOR_OVERLOAD : config.charged ? COLOR_CHARGE : COLOR_ACCENT);
    return bullet;
  }
  ...
  private getBerserkBonus() {
    ...
    const healthRatio = Phaser.Math.Clamp(this.playerStats.health / maxHealth, 0, 1);
    const missing = 1 - healthRatio;
    return Phaser.Math.Clamp(missing * this.berserkConfig.maxBonus, 0, this.berserkConfig.maxBonus);
  }

  private applyInaccuracy(dir: Phaser.Math.Vector2) {
    const maxError = this.bulletHellConfig.active ? this.bulletHellConfig.inaccuracyRad : 0;
  ...
  private payBloodFuelCost() {
    ...
    const cost = this.playerStats.health * this.bloodFuelConfig.fireCostPercent;
    if (cost <= 0) return true;
    this.playerStats.health -= cost;
    this.enforceHealthCap();
    useRunStore.getState().actions.setVitals(this.playerStats.health, this.playerStats.maxHealth);
  ...
  private spawnAfterimageShots(dir: Phaser.Math.Vector2, pilot: PilotRuntime) {
    ...
    for (let i = 1; i <= shots; i++) {
      ...
      const pos = new Phaser.Math.Vector2(pilot.sprite.x + offset.x, pilot.sprite.y + offset.y);
  ...
  private spawnDashSparkExplosion(origin: Phaser.Math.Vector2, dashDir: Phaser.Math.Vector2) {
    ...
    const baseDir =
      dashDir.lengthSq() > 0 ? dashDir.clone().normalize() : new Phaser.Math.Vector2(1, 0);
  ...
  private updateShieldVisual(pilot?: PilotRuntime) {
    if (!pilot) return;
    const active = pilot.shield.hp > 0 && this.time.now <= pilot.shield.activeUntil;
    if (!pilot.shieldRing && active) {
      pilot.shieldRing = this.add
        .arc(pilot.sprite.x, pilot.sprite.y, 34 * OBJECT_SCALE, 0, 360, false, COLOR_ACCENT, 0.2)
  ...
  private handleEnemies() {
    const targetPilot = this.getNearestPilot(GAME_WIDTH / 2, GAME_HEIGHT / 2);
    ...
    this.enemies.getChildren().forEach((child) => {
      ...
      const targetVec = new Phaser.Math.Vector2(pilot.sprite.x - enemy.x, pilot.sprite.y - enemy.y);
      ...
      if (kind === 'drifter') {
        ...
        this.tryEnemyShot(enemy, new Phaser.Math.Vector2(pilot.sprite.x, pilot.sprite.y));
      } else if (kind === 'mass') {
        enemy.setVelocity(
          targetVec.x * speed * 0.7 * slowFactor,
          targetVec.y * speed * 0.7 * slowFactor
        );
        this.tryEnemyShot(enemy, new Phaser.Math.Vector2(pilot.sprite.x, pilot.sprite.y), true);
      } else if (kind === 'boss') {
  ...
  private updateBossMovement(boss: Phaser.Physics.Arcade.Image) {
    ...
    if (targetPilot) {
      ...
      targetY = Phaser.Math.Linear(targetY, targetPilot.sprite.y - 80, 0.08);
    }
  ...
  private bossPatternAimedBurst() {
    ...
    if (!targetPilot) return;
    const center = new Phaser.Math.Vector2(this.boss.x, this.boss.y);
    const target = new Phaser.Math.Vector2(targetPilot.sprite.x, targetPilot.sprite.y)
      .subtract(center)
      .normalize();
  ...
  private bossPatternSlam() {
    ...
    if (!targetPilot) return;
    const center = new Phaser.Math.Vector2(this.boss.x, this.boss.y);
    const toward = new Phaser.Math.Vector2(targetPilot.sprite.x, targetPilot.sprite.y)
      .subtract(center)
      .normalize();
  ...
  private spawnEnemyBullet(
    ...
    (bullet.body as Phaser.Physics.Arcade.Body).setSize(8 * scale, 20 * scale, true);
  ...
  private handleProjectileIntercept(
    ...
    if (!this.lowGraphics) {
      this.spawnBurstVisual(playerBullet.x, playerBullet.y, 18 * OBJECT_SCALE, COLOR_OVERLOAD, 0.6);
    }
  }
  ...
  private triggerChainReaction(
    ...
    if (kind === 'boss') return;
    const damageBase = (maxHealth ?? 0) * this.chainReactionConfig.damagePercent;
    if (damageBase <= 0) return;
    const radius =
      this.chainReactionConfig.radius * (this.activeSynergies.has('black-hole-sun') ? 1.25 : 1);
    this.enemies.getChildren().forEach((child) => {
      const enemy = child as Phaser.Physics.Arcade.Image;
  ...
  private applyBloodFuelHeal() {
    ...
    const healAmount = this.playerStats.maxHealth * this.bloodFuelConfig.healPercent;
  ...
  private applySingularityPull(target: Phaser.Physics.Arcade.Image) {
    ...
    const radius =
      this.singularityConfig.radius * (this.activeSynergies.has('black-hole-sun') ? 1.2 : 1);
    ...
    if (!this.lowGraphics) {
      this.spawnBurstVisual(center.x, center.y, radius * 0.5, COLOR_CHARGE, 0.45);
    }
  }
  ...
  private tryChainArc(origin: Phaser.Physics.Arcade.Image) {
    ...
    if (!this.lowGraphics) {
      const line = this.add
        .line(0, 0, origin.x, origin.y, target.x, target.y, COLOR_PULSE, 0.7)
        .setLineWidth(2);
      this.tweens.add({
        targets: line,
        alpha: { from: 0.7, to: 0 },
  ...
  private handleEnemyDeath(enemy: Phaser.Physics.Arcade.Image, killerTags?: string[]) {
    ...
    const y = enemy.y;
    const maxHealth =
      (enemy.getData('maxHealth') as number | undefined) ?? (enemy.getData('health') as number);
  ...
  private dropXp(x: number, y: number, kind: string) {
    ...
    pickup.setVelocity(this.randBetween(-40, 40), this.randBetween(-40, 40));
    const body = pickup.body as Phaser.Physics.Arcade.Body;
  ...
  private tryShieldPickup() {
    ...
    const activate = (pilot?: PilotRuntime) => {
      ...
      if (!this.lowGraphics) {
        if (!pilot.shieldRing) {
          pilot.shieldRing = this.add
            .circle(pilot.sprite.x, pilot.sprite.y, 26 * OBJECT_SCALE, COLOR_PULSE, 0.06)
        ...
        pilot.shieldRing.setPosition(pilot.sprite.x, pilot.sprite.y);
        this.spawnBurstVisual(pilot.sprite.x, pilot.sprite.y, 30 * OBJECT_SCALE, COLOR_PULSE, 0.9);
      }
    };
  ...
  private spawnCritShrapnel(enemy: Phaser.Physics.Arcade.Image) {
    ...
    for (let i = 0; i < shards; i++) {
      ...
      const dir = new Phaser.Math.Vector2(1, 0).rotate(angle).scale(speedScalar);
      this.spawnBullet({
        x: enemy.x,
        y: enemy.y,
        dir,
  ...
  private enforceHealthCap() {
    ...
    this.playerStats.maxHealth = Math.min(this.playerStats.maxHealth, this.glassCannonCap);
    this.playerStats.health = Math.min(this.playerStats.health, this.playerStats.maxHealth);
  }
  ...
  private applyUpgradeEffects(def: UpgradeDefinition) {
    switch (def.id) {
      ...
      case 'neutron-core': {
        ...
        this.playerStats.projectileSpeed *= this.neutronCoreConfig.speedMultiplier;
        break;
      }
      case 'glass-cannon': {
        ...
        this.playerStats.maxHealth = Math.min(this.playerStats.maxHealth, 1);
        this.playerStats.health = Math.min(this.playerStats.health, this.playerStats.maxHealth);
        useRunStore
          .getState()
          .actions.setVitals(this.playerStats.health, this.playerStats.maxHealth);
      ...
      case 'bullet-hell': {
        ...
        this.playerStats.fireRate *= this.bulletHellConfig.fireRateMultiplier;
        this.playerStats.damage *= this.bulletHellConfig.damageMultiplier;
        break;
      }
      case 'blood-fuel': {
        const stacks = this.upgradeStacks[def.id];
  ...
  private handleWaveIntermission(dt: number) {
    ...
    this.intermissionRemainingMs = Math.max(0, this.intermissionRemainingMs - dt * 1000);
  ...
  private computeWaveScaling(index: number) {
    const overflow = this.infiniteMode && index >= WAVES.length ? index - (WAVES.length - 1) : 0;
    const speedAndFire = this.baseDifficulty * (overflow > 0 ? 1 + overflow * 0.22 : 1);
    const healthScale = this.baseDifficulty * (overflow > 0 ? 1.18 ** overflow : 1);
    const countScale = (overflow > 0 ? 1 + overflow * 0.35 : 1) * this.modeEnemyCountMultiplier;
  ...
  private getWaveSpawns(index: number, countScale: number): EnemySpawn[] {
    ...
    if (index < WAVES.length) {
      return WAVES[index].enemies.map((spawn) =>
        spawn.kind === 'boss'
          ...
          : {
              ...spawn,
              count: Math.max(1, Math.round(spawn.count * countScale)),
            }
      );
    }

    const overflow = index - (WAVES.length - 1);
    const basePoolRaw = WAVES.slice(Math.max(2, WAVES.length - 4), WAVES.length - 1);
    const basePool =
      basePoolRaw.length > 0 ? basePoolRaw : WAVES.slice(0, Math.max(1, WAVES.length - 1));
    const loopIndex = Math.max(0, overflow - 1);
    const template = basePool[loopIndex % basePool.length] ?? WAVES[Math.max(0, WAVES.length - 2)];
    ...
    return template.enemies.map((spawn) => {
      ...
      const elite = spawn.elite !== undefined ? spawn.elite : this.rng.next() < eliteBonusChance;
  ...
  private spawnEnemy(kind: EnemyKind, elite?: boolean) {
    const enemy = this.enemies.get(
      0,
      0,
      kind === 'drifter' ? 'drifter' : kind === 'watcher' ? 'watcher' : 'mass'
    ...
    const stats = getEnemyDefinition(kind, elite);
  ...
  private spawnBoss() {
    ...
    this.boss = this.enemies.get(GAME_WIDTH / 2, 140, textureKey) as Phaser.Physics.Arcade.Image;
    if (!this.boss) return;
    ...
    const health = base.health * (tuning.healthMultiplier ?? 1) * this.enemyHealthScale;
  ...
  private pickPerimeterSpawn() {
    ...
    const edges = [
      {
        x: this.randBetween(this.screenBounds.left, this.screenBounds.right),
      ...
      },
      {
        ...
        y: this.randBetween(this.screenBounds.top, this.screenBounds.bottom),
        side: 'left' as const,
      },
      {
