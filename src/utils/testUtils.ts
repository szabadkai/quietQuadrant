...
export const generators = {
  ...
  filePath: () =>
    fc
      ...
      .chain((dirs) => generators.fileName().map((file) => [...dirs, file].join('/'))),
...
export function createDirectoryOrganizationProperty() {
  return fc.property(generators.filePath(), generators.systemName(), (filePath, _systemName) => {
    const pathParts = filePath.split('/');
    const fileName = pathParts[pathParts.length - 1];

    // System files should be in appropriate directories
    if (fileName.includes('System')) {
      return pathParts.includes('systems') || pathParts.includes('game');
    }
